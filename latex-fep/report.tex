...
As a robustness check, a \texttt{SARIMAX(3,1,0)(0,1,1,12)} model was also estimated with an intervention dummy equal to 1 from March 2020 to December 2022 and 0 otherwise. Dummy (binary) variables Indicator variables (0 or 1) used to isolate anomalous events or structural shifts. A step indicator running in the above window can effectively model the structural demand shock induced by the COVID-19 pandemic, preventing the unusual variance from biasing the model's core autoregressive parameters.
...
\input{annex_errors_2025.tex}

\section{Full STL Decomposition, 2012--2025}
\input{annex_stl_full.tex}
...
\end{minipage}
\vspace{0.3em}
\captionof{figure}{Residual diagnostics for the selected \texttt{SARIMA(3,1,0)(0,1,1,12)} model (left) and the intervention-based \texttt{SARIMAX(3,1,0)(0,1,1,12)} model (right).}
